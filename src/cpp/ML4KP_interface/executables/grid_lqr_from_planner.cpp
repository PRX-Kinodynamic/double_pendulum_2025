...
int main(int argc, char* argv[])
{
  ...
  const std::string dir{ "/Users/Gary/pracsys/remotes/westeros_double_pendulum/src/cpp/ML4KP_interface/acrobot_out/" };
  traj.from_file(dir + "/traj_250217_140229.txt");
  plan.from_file(dir + "/plan_250217_140229.txt");
  PRX_DBG_VARS(plan);
  ...
  gtsam::Values values;
  // GraphValues graph_values{ create_lqr_fg(traj, plan, Xkeys, Ukeys, plant_name) };
  double_pendulum::GainMap Ks{ double_pendulum::create_lqr_fg(traj, plan, values, plant_name) };
  for (auto K_pair : Ks)
  {
    PRX_DBG_VARS(K_pair.first, K_pair.second);
  }
  const bool lqr_traj_ok{ double_pendulum::check_lqr_traj(values, lqr_grid, plan, Ks, sg) };

  PRX_DBG_VARS(lqr_traj_ok);
