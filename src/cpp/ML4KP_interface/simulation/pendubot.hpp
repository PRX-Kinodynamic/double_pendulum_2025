...
class pendubot_t : public prx::plant_t
{
...
public:
  pendubot_t(const std::string& path)
    : prx::plant_t(path)
    , _x(State::Zero())
    , _u(Control::Zero())
    ...
    , _box_w(0.05)
  {
    ...
    state_memory = { &_x[0], &_x[1],  // no-lint
                     &_x[2], &_x[3] };
    state_space = new prx::space_t("RREE", state_memory, "pendubot_state");
    ...
    control_memory = { &_u[0] };
    input_control_space = new prx::space_t("E", control_memory, "pendubot_ctrl");
    input_control_space->set_bounds({ -6 }, { 6 });

    // Derivative memory not used.
    derivative_memory = { &_x[2], &_x[3], &_xdd[0], &_xdd[1] };
    derivative_space = new prx::space_t("EEEE", derivative_memory, "pendubot_dd");

    parameter_memory = {};
    // const std::string param_topology{ std::string(parameter_memory.size(), 'E') };
    parameter_space = new prx::space_t("", parameter_memory, "pendubot_params");
  ...
  void set_params()
  {
    ...
    const double tl1{ 6.0 };
    const double tl2{ 0.0 };
...
PRX_REGISTER_SYSTEM(double_pendulum::pendubot_t, pendubot_dp)
